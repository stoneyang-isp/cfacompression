function rec = interpolation(s,delta_est,phi_est,factor, pixelMask)
...
for k=1:ss(3) % for each color channel
  for i=1:n % for each image
    ...
    %s_c{i} = s_c{i}(:);
    %if ss(1)*ss(2) == size(x)
    %    r{i} = reshape(x, ss(1), [])*factor; % create matrix with row indices
    %    c{i} = reshape(y, ss(1), [])*factor; % create matrix with column indices
    %else
